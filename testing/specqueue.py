...
class specqueue(minqlx.Plugin):
    def __init__(self):
        ...
        self.add_command(("load", "unload"), self.check_for_bots, 1)
    ...
    def handle_player_disconnect(self, player, reason):
        ...
        self.remove_from_queue(player)
        self.remove_from_join(player)
        if not self.bots_loaded or not (self.bot_players[0] or self.bot_players[1] or self.bot_players[2]):
        ...
        if self.q_game_info[0] in SUPPORTED_GAMETYPES and\
            ...
            if self.q_game_info[0] in TEAM_BASED_GAMETYPES:
                ...
                if len(teams["red"]) + len(teams["blue"]) >= self.get_max_players():
                    at_max_players = True
                if self.bots_loaded and (self.bot_players[0] or self.bot_players[1]):
                    try:
                ...
                if len(self.teams()["free"]) >= self.get_max_players():
                    at_max_players = True
                if self.bots_loaded and self.bot_players[2]:
                    add_team = "free"
            if type_action != 0 and (self._queue.size() > 0 or self.game.state in ["in_progress", "countdown"] or
                                     self.fix_sage_active[0] or at_max_players or add_team or
                                     self.red_locked or self.blue_locked):
                try:
                    if player.steam_id not in self._join:
                        self.add_to_join(player)
    ...
    def handle_game_start(self, data):
        ...
        if self.get_cvar("qlx_queueAutoSargeFix", bool):
            if not self.get_cvar("qlx_bdmBalanceAtGameStart", bool) and self.get_cvar("qlx_bdmEnableSwitch", bool):
                self.sarge_bug()
    ...
    def get_rating(self, sid):
        ...
        if self.db.exists(BDM_KEY.format(sid, game_type, "rating")) == 1:
    ...
    def check_for_bots(self):
        self.bots_loaded = True if "bots" in minqlx.Plugin._loaded_plugins else False
